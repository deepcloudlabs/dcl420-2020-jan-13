...
//        });
    ...
    getJson() {
        ...
        raw.photo = toRawImage(raw.photo);
        delete raw.isValid;
...
class HRViewModel {
    constructor() {
        ...
        this.departments = ko.observableArray(["SALES", "ACCOUNTING", "IT", "HR"]);
    ...
    findEmployeeByIdentity() {
        $.ajax({
            method: "GET",
            url: AppConfig.URL+"/api/v1/employees/" + this.employee.identityNo(),
            cache: false,
            headers: {
    ...
    removeEmployeeByIdentity(identity) {
        $.ajax({
            method: "DELETE",
            url: AppConfig.URL+"/api/v1/employees/" + identity,
            success: (emp) => {
                this.employee.update(emp);
                if (emp === undefined)
                    ...
                    const notDeleted =
                        e => e.identityNo.localeCompare(identity) != 0;
    ...
    deleteEmployee(emp) {
        this.removeEmployeeByIdentity(emp.identityNo);
    }

    deleteEmployeeByIdentity() {
        this.removeEmployeeByIdentity(this.employee.identityNo());
    }
    ...
    addEmployee() {
        ...
        $.ajax({
            method: "POST",
            url: AppConfig.URL+"/api/v1/employees",
    ...
    findAllEmployees() {
        $.ajax({
            method: "GET",
            url: AppConfig.URL+"/api/v1/employees?page=0&size=100",
            headers: {
            	'Accept' : 'application/json'
            },
            cache: false,
            success: (employees) => {
                this.populatePhotoIfNull(employees);
